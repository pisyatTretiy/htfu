...
export class FishingScene3D {
  ...
  applyZone(zone: Zone): void {
    ...
    this.pier.applyZone(zone.decor.set);
  ...
  private label(entry: CatchEntry): string {
    ...
    // Прилагательное согласуется и с языком, и с родом названия: «Редкий
    // boot» английский игрок видел до первой правки, «Редкий Плотва» русский —
    // до второй.
    return i18n.t(`rarity.${this.rarity}.${entry.gender}`, { name: name.toLowerCase() });
  }
