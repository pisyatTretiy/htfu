 ...
 * улов буянит на настиле → усмирение. Снимает кадры для журнала спайка и служит
...
/** Улов буянит: тапаем по всему настилу — он прыгает, с первого раза не попасть. */
...
async function main(): Promise<void> {
  ...
  // Показ улова на леске длится полторы секунды: снимаем его, пока висит.
  await page.waitForTimeout(500);
  await page.screenshot({ path: `${OUT}/5a-showcase.png` });
  ...
  // а после возни на настиле никто не забрасывает сам.
